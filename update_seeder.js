...
const foodWords = ['bitter', 'coconut', 'egg', 'eggplant', 'food', 'fruit', 'honey', 'meat', 'milk', 'salt', 'sugar', 'sweet', 'water', 'rice', 'banana', 'cassava', 'taro', 'yam', 'drink', 'eat', 'cook'];
...
content = content.replace(/english = "([^"]+)",\s*category = "[^"]+"/g, (match, eng) => {
    const cat = getCategory(eng);
    return `english = "${eng}",\n            category = "${cat}"`;
});

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully updated DatabaseSeeder.kt with Node!');
