import { 
  db, collection, doc, setDoc, addDoc, updateDoc, deleteDoc, query, orderBy, onSnapshot 
...
// Global window tab switcher function (Bulletproof)
...
function initRealtimeListeners() {
  try {
    const subQuery = query(collection(db, "word_submissions"), orderBy("submittedAt", "desc"));
    onSnapshot(subQuery, (snapshot) => {
      submissions = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      renderSubmissionsTable();
      updateDashboardMetrics();
    }, (error) => {
      console.warn("Submissions listener error:", error);
  ...
  try {
    ...
    onSnapshot(vocabQuery, (snapshot) => {
      ...
      renderVocabularyError("Unable to connect to live Firestore master dictionary.");
    });
  } catch (e) {
  ...
  try {
    const releaseQuery = query(collection(db, "app_releases"), orderBy("versionCode", "desc"));
    onSnapshot(releaseQuery, (snapshot) => {
      releases = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      renderReleasesList();
      updateDashboardMetrics();
    }, (error) => {
      console.warn("Releases listener error:", error);
    });
  } catch (e) {
...
// Render Master Vocabulary Table
...
function initSqlImporter() {
  ...
  dropzone.addEventListener('drop', (e) => {
    ...
    if (e.dataTransfer.files.length > 0) {
      handleSqlFile(e.dataTransfer.files[0]);
    }
  });

  fileInput.addEventListener('change', (e) => {
    if (e.target.files.length > 0) {
      handleSqlFile(e.target.files[0]);
    }
  });
}
...
function initFormListeners() {
  ...
  if (addVocabForm) {
    addVocabForm.addEventListener('submit', async (e) => {
      ...
      if (!word) {
        return alert("Please enter the Kasiguranin word.");
      }

      const isDuplicate = vocabulary.some(v => (v.kasiguranin || '').toLowerCase() === word.toLowerCase());
      if (isDuplicate) {
        if (!confirm(`"${word}" already exists in the master dictionary. Do you want to add it anyway?`)) {
          return;
        }
      }
  ...
  if (releaseForm) {
    releaseForm.addEventListener('submit', async (e) => {
      ...
      if (isNaN(code) || code <= 0) {
        return alert("Please enter a valid positive integer version code (e.g. 1, 2, 3).");
      }
      if (!name) {
        return alert("Please enter a version name (e.g. 1.0.0).");
      }
      if (!url.startsWith('http://') && !url.startsWith('https://')) {
        return alert("Direct APK Download link must start with http:// or https://");
      }
